...
public class PanelJuego extends JPanel implements ActionListener, KeyListener {
    ...
    public PanelJuego(String nombre1, String nombre2) {
        ...
        setFocusable(true);
        addKeyListener(this); // Hace que esté escuchando las teclas desde este objeto

        try {
            fondo = ImageIO.read(new File("Free-Project/Free-Project/src/resources/fondo.png"));
        } catch (IOException e) {
            e.printStackTrace();
        }

        try {
            fuente = Font.createFont(Font.TRUETYPE_FONT, new File("Free-Project/Free-Project/src/resources/Poppins-Regular.ttf")).deriveFont(36f);
    ...
    protected void paintComponent(Graphics g) {
        super.paintComponent(g); //  llama al metodo paintComponent de la superclase de PanelJuego
        ...
        // Definir donde esta cada paleta del jugador
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (jugador1Arriba && jugador1Y > 0) {
            jugador1Y -= velocidadPaleta; // Mueve la paleta hacia arriba
        } else if (jugador1Abajo && jugador1Y + altoPaleta < ALTO) {
            jugador1Y += velocidadPaleta; // Mueve la paleta hacia abajo
        }

        // Movimiento del Jugador 2
        if (jugador2Arriba && jugador2Y > 0) {
            jugador2Y -= velocidadPaleta; // Mueve la paleta hacia arriba
        } else if (jugador2Abajo && jugador2Y + altoPaleta < ALTO) {
            jugador2Y += velocidadPaleta; // Mueve la paleta hacia abajo
        }

        pelotaX += velocidadX; // Actualiza la posición horizontal de la pelota
        pelotaY += velocidadY; // Actualiza la posición vertical de la pelota
        ...
        // Condiciones de juego para los puntos
        if (pelotaX < 0) {
            puntosJugador2++;
        ...
        // Reinicia la pantalla de los elementos creados
        repaint();
    }
    ...
    private void reiniciarPelota() {
        ...
        // Enviar random la pelota con un 50% de posibilidades a un lado
        if (Math.random() > 0.5) {
            velocidadX = Math.abs(velocidadX);  // Positiva
        } else {
            velocidadX = -Math.abs(velocidadX); // Negativa
        }
    }
